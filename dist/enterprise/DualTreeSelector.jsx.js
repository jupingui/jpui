...
define(function (require) {
    ...
    return React.createClass({
        ...
        componentWillReceiveProps: function componentWillReceiveProps(nextProps) {
            if (nextProps.isDropDown && !util.isEqual(JSON.parse(nextProps.value || '{}'), JSON.parse(this.state && this.state.dropdownValue ? this.state.dropdownValue : '{}')) && this.state && !this.state.___beOperated___) {
                this.setState({ dropdownValue: nextProps.value });
            }
        },
        onTreeChange: function onTreeChange(e) {
            this.___dispatchChange___(e);
        },
        onTreeAction: function onTreeAction(type, args) {
            if (type === 'TreeExpandNode') {
                this.setState({ expand: args.expand });
            }
            typeof this.props.onAction === 'function' && this.props.onAction(type, args);
        },
        onDropDownTreeChange: function onDropDownTreeChange(e) {
            this.setState({
                dropdownValue: e.target.value,
                ___beOperated___: true
            });
        },
        onLayerRender: function onLayerRender() {
        ...
        appendAll: function appendAll(e) {
    ...
    function mainContentFactory(me) {
        ...
        var selectedInfo = labels.footSelectCountTpl ? labels.footSelectCountTpl.replace(/&selected/g, selected).replace(/&total/g, count) : null;
        ...
        return React.createElement(
            'div',
            containerProp,
            labels.title ? React.createElement(
                'div',
                { className: 'selector-title-bar' },
                labels.title
            ) : null,
            React.createElement(
                'div',
                { className: 'tree-title-bar' },
                React.createElement(
                    'div',
                    null,
                    labels.leftTreeTitle ? labels.leftTreeTitle : null
                ),
                React.createElement(
                    'div',
                    null,
                    labels.rightTreeTitle ? labels.rightTreeTitle : null
                )
            ),
            React.createElement(DualTreeSelector, treeProp),
            React.createElement(
                'div',
                { className: 'tree-foot-bar' },
                React.createElement(
                    'div',
                    null,
                    React.createElement(
                        'span',
                        { className: 'remove-all-button', onClick: me.appendAll },
                ...
                React.createElement(
                    ...
                    React.createElement(
                        'span',
                        { className: 'remove-all-button', onClick: me.deleteAll },
