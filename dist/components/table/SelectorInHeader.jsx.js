    ...
    return React.createClass({
        ...
        getDefaultProps: function getDefaultProps() {
            ...
            return {
                fieldConfig: {},
                tableComponent: {},
                datasource: [{ label: language.tableSelector.selectCurrentPage, value: '-2' }, { label: language.tableSelector.selectAll, value: '-1' }]
            };
        },
        // @override
        ...
        onMainSelectorChange: function onMainSelectorChange(e) {
            var table = this.props.tableComponent;
            var info = getInformationFromTable(table);
            e = { target: this.refs.container };
            if (info.workMode === SELECT_MODE.CURRENT_PAGE) {
                e.target.value = info.i === table.props.datasource.length ? SELECT_COMMAND.CLEAR : SELECT_COMMAND.CURRENT_PAGE;
            } else {
                // 如果是全选或当页全选，则取消所有选择；否则选中当前页
                e.target.value = info.i === -1 || info.i === table.props.datasource.length ? SELECT_COMMAND.CLEAR : SELECT_COMMAND.CURRENT_PAGE;
            }
            this.props.tableComponent.onRowSelect(e);
        },
        render: function render() {
            ...
            var checked = info.i === -1 || info.workMode === SELECT_MODE.CURRENT_PAGE && info.i === table.props.datasource.length;
            table.props.datasource.length === 0 && (checked = false);
            var indeterminate = info.workMode === SELECT_MODE.CURRENT_PAGE ? info.i > 0 && info.i < table.props.datasource.length : info.i !== -1 && info.i > 0;
            ...
            if (info.workMode === SELECT_MODE.CURRENT_PAGE || info.workMode === SELECT_MODE.ALL) {
                return React.createElement(
                    'th',
                    { className: 'th-header table-selector', ref: 'container' },
                    React.createElement('span', virtualCheckboxProps)
                );
            }
            ...
            return React.createElement(
                'th',
                { className: 'th-header table-selector', rowSpan: this.props.rowSpan },
                React.createElement(
                    'div',
                    containerProp,
                    ...
                    React.createElement(
                        Layer,
                        layerProp,
                        React.createElement(List, { datasource: this.props.datasource, onClick: this.onListClick })
                    )
                )
            );
        }
