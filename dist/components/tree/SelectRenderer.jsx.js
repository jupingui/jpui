...
define(function (require) {
    ...
    return React.createClass({
         ...
         */
        onOperate: function onOperate(e) {
            e.stopPropagation();
        ...
        render: function render() {
            ...
            // 节点所在子树的选择状态
            var select = tools.getNodeSelectInfo(item, me.props.value.selected || {}, this.props.index, this.props.treeComponent.___loadingCache___);
            // 显示状态
            ...
            if (display === 'all') {
                ...
                selectBtnProp = {
                    className: 'select-button jpui-icon ' + (disabled ? ' select-button-disabled' : '') + (select.selected === select.total && select.total > 0 ? ' jpui-icon-check' : ' jpui-icon-plus'),
            ...
            return React.createElement(
                'div',
                containerProp,
                React.createElement('div', selectBtnProp),
                React.createElement('div', iconProp),
                React.createElement(
                    'span',
                    null,
                    item.label
